using DALHelperCore.Helpers.Persistence;
using DALHelperCore.InternalClasses.Helpers.Operations;
...
namespace DALHelperCore.InternalClasses.Helpers.DataTransfer.Persistence
{
	internal class DataOutputOperations
	{
		...
		internal static int BulkTableWrite<T>(MySqlConnection ExistingConnection, T SourceData, string TableName = null, MySqlTransaction SqlTransaction = null, Type ForceType = null, bool AllowUserVariables = false)
		{
			var rowsUpdated = GetBulkTableWriter<T>(ExistingConnection)
				.SetTableName(TableName ?? DALHelper.GetDalTable(ForceType ?? typeof(T)) ?? throw new CustomAttributeFormatException(DatabaseCoreUtilities.NoDalTableAttributeError))
				.SetSourceData(SourceData)
				.UseTransaction(true)
